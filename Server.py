...
class TCP_SERVER:
                ...
                while True:
                    ...
                    if data:
                        ...
                        if client_address[0] not in self.IPS:
                            t = Thread(target=self.thread_trigger, args=(client_address,))
                            t.start()
    ...
    def thread_trigger(self, c_address):
        # creating queue
        connection = pika.BlockingConnection(
            pika.ConnectionParameters(host='192.168.1.14'))  # Connect to CloudAMQP
        channel = connection.channel()  # start a channel
        channel.queue_declare(queue='gia')  # naming queue
        channel.basic_publish(exchange='', routing_key='gia', body=bytes(c_address[0]))
        channel.basic_publish(exchange='', routing_key='gia', body=bytes(str(self.IDS_size)))
