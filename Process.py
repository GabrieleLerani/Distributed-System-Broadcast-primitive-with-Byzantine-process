        ...
        match message.get("TYPE"):
            ...
            case 2:
                ...
                if len(temp_l) == len(self.ids) - self.faulty and not self.delivered:
                    number_of_checks = (round.len(temp_l) + 1) // 4
                    bar = threading.barrier(parties = 4)
                    check_threads = []
                    # one is already running so only 3 threads are created
                    for num in range(3):
                        check_threads[num] = Thread(target=self.thread_check, args=
                                          (temp_l[number_of_checks * num : min(number_of_checks * (num + 1), len(temp_l) - 1)])
                                          )
                        check_threads[num].start()
                    # TODO
                    # if num = 3 after the for cycle has closed then it is fine
                    # if num = 2 after the for cycle has closed, then below num variables should be num + 1 (and num + 1 should be num + 2)
                    check_threads[num] = threading.current_thread()
                    self.thread_check((temp_l[number_of_checks * num : min(number_of_checks * (num + 1), len(temp_l) - 1)]))
                    bar.wait()
                    # all threads, except the first one (the one with the lowest number), are closed
                    if threading.current_thread() != min(check_threads):
                        return
                    if self.counter == len(self.ids) - self.faulty:
                        for i in range(len(self.ids)):
                            self.L[i].link_send(message)
                            # TODO
                            # all messages should have the same MSG so picking the first one should be correct (?)
                            self.deliver(temp_l[0]["MSG"])
                            self.__close_link()
