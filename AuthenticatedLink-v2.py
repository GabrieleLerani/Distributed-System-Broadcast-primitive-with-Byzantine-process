...
class AuthenticatedLink:
    ...
    def __receive(self):
        ...
        port = int("50" + self.id + self.self_id) if self.self_id < 10 and self.id < 10 else int(
            "5" + self.id + self.self_id)
    ...
    def __auth(self, message, flag):
        ...
        mess = {"MSG": message,
                "HMAC": hmac.new(self.key[self.id], flag + message, hashlib.sha256).hexdigest(),
                "FLAG": flag}
        return mess
    ...
    def send(self, message, flag):
        # It uses ternary operator
        port = int("50" + self.self_id + self.id) if self.self_id < 10 and self.id < 10 else int(
            "5" + self.self_id + self.id)
