...
if __name__ == "__main__":
    ...
    if ((len(sys.argv) == 4) and (sys.argv[3] == 'R' or sys.argv[3] == 'B')) or (
            (len(sys.argv) == 3) and (sys.argv[2] == 'R')):
        ...
        match check_variable:
            ...
            case "3":
                ...
                p = Process_2.Process()  # 3

            case "4":
                logging.info("MAIN:STARTING PROTOCOL--> ERASURE CODE BASED BROADCAST")
                p = Process_2.Process()  # 4

            case _:
                print(
                    "Usage: python main.py <message to broadcast> <protocol number> <process type>: protocol number must be in this list[1,2,3,4]")
                exit(-1)
        ...
        if (len(sys.argv) == 4) and sys.argv[3] == 'B':
            ...
            time.sleep(5)
            p.broadcast(message)
        logging.info("MAIN:-----SUCCESSFULLY EXIT-----")
        exit(0)
    elif len(sys.argv) < 4:
        print(
            "Usage: python main.py <message to broadcast> <protocol number> <process type>: too few arguments: 4 required:",
            len(sys.argv) - 1, "given")
        exit(-1)

    elif not (sys.argv[3] == 'R' or sys.argv[3] == 'B'):
        print(
            "Usage: python main.py <message to broadcast> <protocol number> <process type>: process type must be in this list[B,R]")
        exit(-1)
    else:
        print(
            "Usage: python main.py <message to broadcast> <protocol number> <process type>: too many arguments: 4 required:",
            len(sys.argv) - 1, "given")
        exit(-1)
