...
SERVER_ID = "192.168.1.115"
SERVER_PORT = 5000

KDS_IP = '192.168.1.115'
KDS_PORT = 8080
...
class Process:
        ...
        match message.get('TYPE'):
            ...
            case 1:
                ...
                if message.get('FLAG') == "VOTE" and self.check_signature(message.get('FLAG') + message.get('MSG'), message.get('SIGN'),
                                                                      message.get('FROM')):
                    self.signed_vote_messages_container[self.sid][str(message.get('FROM'))][str(message.get('MSG'))] = message.get(
                        'MSG')
                    ...
                    for id in message['keys'].keys():
                        if PLACEHOLDER not in id:
                            ...
                            if counter == len(self.ids) - self.f:
                                self.deliver(message['keys'][str(id) + PLACEHOLDER]['MSG'])
                else:
    ...
    def check_signature(self, message, signature, idn):
        ...
        # TODO this logging fails for b boolean
