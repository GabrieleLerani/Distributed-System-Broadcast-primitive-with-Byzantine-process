...
class Process:
        ...
        while True:
            if i not in self.checked_indexes:
                ...
                if final_msg_dict.get('GO'):
                    ...
                    for j in range(0, len(self.ids)):
                        self.L[j].thread_s.terminating_flag=True
                        self.L[j].thread_s.terminating_flag = True
                        self.AL[j].terminating_flag=True
    ...
    def process_receive(self, msg):
        ...
        match msg.get('TYPE'):
            ...
            case 1:
                ...
                if msg.get('FLAG') == "VOTE" and self.check_signature(msg.get('FLAG') + msg.get('MSG'), msg.get('SIGN'),
                                                                      msg.get('FROM')):
                    self.signed_vote_messages_container[self.sid][str(msg.get('FROM'))][str(msg.get('MSG'))] = msg.get(
                        'MSG')
                    self.signed_vote_messages_container[self.sid][str(msg.get('FROM'))][
                        str(msg.get('MSG')) + 'SIGN'] = msg.get('SIGN')
                else:
                    if msg.get('FLAG') != "VOTE":
                        logging.info("PROCESS:Wrong flag received %s", msg.get('FLAG'))
                    else:
            ...
            case 2:
                ...
                if not self.delivered:
                    ...
                    for id in list(msg['keys'].values()):
                        if self.check_signature(msg['keys'][str(id) + PLACEHOLDER]['MSG'],
                                                msg['keys'][str(id) + PLACEHOLDER]['SIGN']):
                            counter = counter + 1
    ...
    def count(self, i):
        ...
        checked_msgs = []
        for id in range(1, len(
                self.signed_vote_messages_container[i]) + 1):  # checking for all processes id that sent to i a message
            counter = 0
            ...
            current_values = self.signed_vote_messages_container[i][
                str(id)].values()  # listing messages received from id
            for current_msg in current_values:  # choosing one message received from id
                ...
                if current_msg not in checked_msgs:
                    for id2 in range(1, len(
                            self.signed_vote_messages_container[i]) + 1):  # choosing another process different from id
                        if id2 != id:
                            ...
                            for other_msg in current_values2:  # checking one message between those that process id2 sent to i
                                if current_msg == other_msg:
                                    ...
                                    current_dict['keys'][str(id2) + PLACEHOLDER]['SIGN'] = \
                                        self.signed_vote_messages_container[i][str(id2)][other_msg + 'SIGN']
                                    counter = counter + 1
                                if counter == (len(self.ids) - self.f):
